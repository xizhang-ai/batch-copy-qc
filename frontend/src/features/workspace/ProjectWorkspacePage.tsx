...
import type { AssistantPlan, AssistantSession, CopyType, GenerationRun, Project } from "../../api/contracts";
...
export function ProjectWorkspacePage() {
  ...
  return <div className="page workspace-page">
    <div className="workspace-heading"><div><div className="eyebrow">任务工作台</div><h1>{project?.name || "正在打开任务"}</h1><p className="page-description">说清楚你想做什么，系统会把复杂配置整理成可执行的任务；你也可以随时直接打开高级配置。</p></div><div className="inline-actions"><Link className="button button-secondary" to={`/projects/${id}/types`}>高级配置</Link><Link className="button button-secondary" to={`/projects/${id}/board`}>查看全部文案</Link></div></div>
