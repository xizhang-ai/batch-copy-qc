...
export function CopyEditor({ value, onChange, onSelection, titleRef, bodyRef, tagRef }: { value: CopyItem; onChange: (value: CopyItem) => void; onSelection: (selection?: SelectionState) => void; titleRef: RefObject<HTMLTextAreaElement | null>; bodyRef: RefObject<HTMLTextAreaElement | null>; tagRef: RefObject<HTMLInputElement | null> }) {
  ...
  return <div className="copy-editor form-stack">
    <div className="field"><label htmlFor="review-title">标题</label><textarea ref={titleRef} id="review-title" className="textarea title-input" rows={2} value={value.title} onChange={(event) => onChange({ ...value, title: event.target.value })} onSelect={(event) => capture("title", event.currentTarget)} onContextMenu={(event) => context(event, "title")} /></div>
