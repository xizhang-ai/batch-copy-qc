...
export function CopyEditor({ value, onChange, onSelection, titleRef, bodyRef }: { value: CopyItem; onChange: (value: CopyItem) => void; onSelection: (selection?: SelectionState) => void; titleRef: RefObject<HTMLInputElement | null>; bodyRef: RefObject<HTMLTextAreaElement | null> }) {
  ...
  return <div className="copy-editor form-stack">
    ...
    <div className="field"><label htmlFor="review-tags">话题标签</label><div className="chip-input"><Icon name="plus" /><input id="review-tags" value={value.tags.join("、")} onChange={(event) => onChange({ ...value, tags: event.target.value.split(/[、,#\s]+/).filter(Boolean) })} placeholder="输入后用空格或顿号分隔" /></div><div className="chip-row">{value.tags.map((tag) => <span className="source-chip" key={tag}>#{tag}</span>)}</div></div>
  </div>;
}
