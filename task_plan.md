# 批量种草文案 QC 独立产品：规划任务

## 目标

按照已确认的完整实施计划，构建并验收一个可在 Windows 本地运行的独立批量种草文案 QC 产品，覆盖项目/帖子类型 Brief、爆款参考、批量生成、自动 QC、人工审核、飞书子 Sheet 输出和完整审计链。

## 当前阶段

- 状态：本轮完成
- 阶段：任务工作台端到端回归、修复与真实页面核验已完成
- 编码边界：保持独立产品与轻量单体；子智能体按目录所有权工作，主智能体负责交叉集成、实网适配和最终验收。

## 阶段

- [x] 阶段 1：确认产品边界与已冻结决策
- [x] 阶段 2：核对现有短策划、技术选型和视觉参考
- [x] 阶段 3：锁定文件结构、领域模型、状态机与接口边界
- [x] 阶段 4：编写逐任务实施计划
- [x] 阶段 5：把爆款参考类型补入需求、数据、API、QC、前端和验收后完成自检
- [x] 阶段 6：形成长任务执行交接点并停止，不启动编码
- [x] 阶段 7：初始化独立仓库和应用骨架
- [x] 阶段 8：完成后端领域、持久化、模型/QC/审核/飞书 API
- [x] 阶段 9：完成前端项目、类型、规则、看板、审核和输出页面
- [x] 阶段 10：全栈联调、实网验证、E2E、Windows 启动与文档收口
- [x] 阶段 11：独立审查官终审与问题修复
- [x] 阶段 12：将看板改为系统处理中、需人工处理、已完成三个用户视图
- [x] 阶段 13：完成对话驱动项目配置与三篇预览生成的实施计划
- [x] 阶段 14：按 2026-08-24 对话与预览计划实施并验收
- [x] 阶段 15：内置“红书种草写作策略 v1”，且仅在生成时调用
- [x] 阶段 16：防御任务助手的不完整规则动作，避免应用任务时中断
- [x] 阶段 17：全面排查任务工作台链路，并修复对话动作、生成前校验和批次状态呈现

## 已冻结决策

- 产品是独立产品，位于 `E:\xixiAi\batch-copy-qc`，只适量参考其他项目。
- 技术栈：React + TypeScript + Vite；Python 3.12 + FastAPI + Pydantic；SQLite；RapidFuzz。
- 用户输入项目 Brief；模型拆解后可视化，所有字段可修改。
- 文案类型无内置预设，由用户新增空白类型，可再次输入或上传类型 Brief。
- 帖子类型也可由用户提供 1–5 篇爆款参考帖建立；系统完整保存原帖，AI 抽取可编辑风格画像，生成 Prompt 同时携带原帖与画像，但不得继承来源事实或照搬独特原句。
- 用户未单独输入类型 QC 时，“一定要有/一定不要有”自动成为可编辑的默认类型 QC。
- 模板入口仅预留数据边界，本阶段不做模板库。
- QC 分项目级、类型级、生成后的个例人工处理；硬规则不可覆盖，软规则允许类型覆盖项目。
- AI QC 通过可直接进入已完成；异常项才进入人工审核。
- 自动 QC 发起的 AI 修改完成后重新 AI QC；人工发起的定向修改完成后返回人工审核。
- 强制通过必须记录非空遗留问题。
- 每次飞书输出在同一电子表格中新建子 Sheet；API 与飞书标识由 `.env` 提供。
- 模型 API 供应商固定为 CLIPROXY；Base URL、API Key、生成/QC 模型与超时从 `.env` 读取，开发测试保留 Fake Adapter。
- 首版不做拖拽，不设置“一轮修改/二轮修改”列，修改次数作为元数据。
- 内容质量优化属于 P1，本计划先搭建可运行、可验证的轻量框架。

## 错误与风险记录

| 项目 | 状态 | 处理 |
|---|---|---|
| 早期方案曾过度向另一个 TypeScript 项目对齐 | 已纠正 | 保持独立 Python/FastAPI 产品，仅参考交互与接口模式 |
| 视觉规范中“人工审核是唯一出口”与最新流程冲突 | 已纠正 | 以最新决定为准：AI QC 通过可直接完成 |
| 参考案例画像与“原帖必须进入 Prompt”存在早期口径差异 | 已纠正 | 原帖完整保存并进入 Prompt，同时保留画像、事实隔离和防照搬 QC |
| PowerShell 将 if/foreach 结果直接接管道时出现空管道解析错误 | 已解决 | 先赋值到 `$results`，再单独送入 `Format-Table`；后续脚本沿用此模式 |
| 首次 Git 提交缺少 author identity | 已解决 | 仅为本仓库设置 `Codex <codex@local>`，不修改用户全局 Git 配置 |
| PowerShell `Start-Process` 重定向路径直接拼接导致参数绑定失败 | 已解决 | 先把日志绝对路径保存为独立变量，再传给 RedirectStandardOutput/Error |
| Playwright 技能的 Bash wrapper 在本机无 `/bin/bash` | 已解决 | 保持 CLI-first，改用同等的 `npx --package @playwright/cli playwright-cli` 调用 |
| 单个 apply_patch 同时 Delete/Add 同一路径被拒绝 | 已解决 | 分成先删除、再新增两个补丁，不重复原失败形式 |
| 审查修复后 Ruff 报 `feishu.py` import 顺序 | 已解决 | 使用 Ruff 单文件自动修复，并重跑全目录检查 |
| `npm --prefix ... exec` 从仓库根解析了错误的 E2E 配置相对路径 | 已解决 | 固定为前端 `test:e2e` script，并让 Playwright webServer 的 cwd 指向仓库根 |
| Playwright 升级后缺少对应 Chromium 1234 | 已解决 | 安装项目测试浏览器；失败 trace 位于 Git 忽略目录 |
| E2E 两处定位器受内联空格和重复“人工通过”影响 | 已解决 | 使用空白容忍正则与 ITEM ID 范围定位，不改业务 UI |
| Windows PowerShell 5 误解码无 BOM UTF-8 中文脚本 | 已解决 | 三个交付脚本自身只使用 ASCII 文本，兼容旧版和新版 PowerShell |
| `py -3.12` 未在 Windows Launcher 注册 | 已解决 | 脚本优先使用项目 `.venv`；已用 bundled Python 3.12 建立本地 `.venv` |
| 类型 Brief 的真实 API 结果没有回填类型编辑字段 | 已解决 | 后端返回可编辑 requirements/约束/项目建议，前端显示拆解与冲突提示且不覆盖项目事实 |
| 任务工作台改版后旧 E2E 假设 Brief 表单自动展开 | 已解决 | E2E 改为显式点击“上传 Brief”，并覆盖新工作台到高级配置的真实路径 |
