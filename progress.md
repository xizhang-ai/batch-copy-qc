...
- 根据用户反馈修正工作台入口：不能以对话替代 Brief。工作台已恢复“上传 Brief”（粘贴/上传文件→AI 拆解→明确确认写入）并提供“完整 Brief 编辑”直达原有逐条编辑页；对话仅作为补充输入，原项目内容、类型、规则、审核和输出链路保留。

## 2026-08-25

- 已内置全局 **红书种草写作策略 v1**：纯指令构建器仅在 `GenerationWorker.process()` 的生成分支、调用模型前注入。它不参与 Brief 解析、对话助手、看板、导出或 QC。
